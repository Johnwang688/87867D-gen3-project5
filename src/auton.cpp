#include "auton.hpp"

namespace bot {

    namespace drivetrains {
        bot::Drivetrain dt = bot::Drivetrain(bot::motors::left_dt, bot::motors::right_dt, bot::sensors::imu);
    }

    namespace mcl {
        bot::Location location;
    }

    namespace autons {
        using namespace bot::drivetrains;

        namespace {
            void reset_tracking_for_test(double x, double y, double heading) {
                if (bot::mcl::location.is_running()) {
                    bot::mcl::location.stop();
                }
                bot::mcl::location.reset(x, y, heading);
                bot::mcl::location.start();
            }

            void stop_tracking_if_running() {
                if (bot::mcl::location.is_running()) {
                    bot::mcl::location.stop();
                }
            }
        }

        void left_7() {
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            double start_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.print("start time: %.1f", start_time);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::pistons::arm_pistons.set(true);
            dt.drive(650, 1500, 100, -35);
            bot::pistons::match_load_piston.set(true);
            dt.drive(320, 1000, 50, -150);
            dt.drive(600, 1500, 70, -135);
            dt.drive(1000, 1400, 50, 180);
            dt.drive(-200, 800, 30, 180);
            dt.drive(-500, 1000, 60, 180);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            bot::pistons::match_load_piston.set(false);
            dt.drive(-200, 500, 40, 180);
            dt.brake();
            vex::task::sleep(1200);
            dt.coast();
            dt.drive(25, 400, 30, 180);
            dt.drive(170, 1500, 50, -90);
            bot::pistons::arm_pistons.set(false);
            dt.turn_to_heading(180, 500, 100);
            dt.drive(-500, 1500, 60, 180);
            dt.brake();
            bot::Controller1.Screen.setCursor(2,1);
            double end_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.print("end time: %.1f", end_time);
            bot::Controller1.Screen.setCursor(3,1);
            bot::Controller1.Screen.print("time taken: %.1f", end_time - start_time);
            return;
        }

        void left_4_3() {
            double start_time = vex::timer::system();
            bot::sensors::imu.setHeading(0.0, vex::degrees);
            bot::pistons::arm_pistons.set(true);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(650, 1500, 80, -25);
            bot::pistons::match_load_piston.set(true);
            dt.drive(-300, 1500, 80, 60);
            dt.drive(-200, 1000, 80, 90);
            bot::pistons::match_load_piston.set(false);
            dt.drive_dist(510, 1000, 70, 90, 5.0, bot::sensors::back_dist, bot::rev);
            dt.drive(-500, 800, 50, 180);
            bot::pistons::hood_piston.set(true);
            dt.drive(-400, 500, 70, 180);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.brake();
            vex::task heading_adjust_task = vex::task([]() -> int {
                dt.drive(25, 300, 50, 180);
                dt.turn_to_heading(180, 500, 100);
                dt.drive(-400, 500, 50, 180);
                return 0;
            });
            vex::task::sleep(1500);
            dt.coast();
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::motors::upper.spin(vex::reverse, 4.0, vex::volt);
            dt.drive(200, 1000, 30, 180);
            bot::pistons::hood_piston.set(false);
            bot::pistons::match_load_piston.set(true);
            dt.drive(550, 1200, 60, 180);
            vex::task::sleep(200);
            dt.drive(300, 300, 70, 180);
            vex::task::sleep(100);
            dt.drive_for(-100, 800, 80, 180);
            dt.turn_to_heading(-135, 1000, 100);
            dt.drive(-1400, 2500, 80, -135);
            bot::pistons::mid_piston.set(true);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::motors::upper.spin(vex::reverse, 100, vex::percent);
            dt.drive(-200, 800, 50, -135);
            dt.brake();
            vex::task::sleep(1000);
            dt.coast();
            dt.drive(200, 800, 60, -135);
            dt.brake();
            bot::pistons::mid_piston.set(false);
            vex::task::sleep(200);
            dt.drive(-300, 700, 80, -135);
            bot::motors::intake.stop();
            vex::task::sleep(200);
            dt.drive_for(1000, 2000, 80, -135);
            dt.turn_to_heading(180, 1000, 100);
            bot::pistons::arm_pistons.set(false);
            dt.drive(-500, 1000, 100, 180);
            dt.hold();
            double end_time = vex::timer::system();
            bot::Controller1.Screen.setCursor(2,1);
            bot::Controller1.Screen.print("time taken: %.1f", end_time - start_time);
            return;
            /*
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            double start_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.print("start time: %.1f", start_time);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::pistons::arm_piston.set(true);
            dt.drive(650, 1500, 100, -35);
            bot::pistons::match_load_piston.set(true);
            dt.drive(-480, 1500, 70, 60);
            dt.drive(-900, 1200, 50, 180);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            vex::task heading_adjust_task = vex::task([]() -> int {
                dt.turn_to_heading(180, 1000, 100);
                return 0;
            });
            vex::task::sleep(1500);
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(200, 500, 20, 180);
            dt.drive(400, 800, 50, 180);
            dt.drive(700, 1000, 40, 180);
            dt.drive(-120, 800, 50, 180);
            dt.drive(-400, 1000, 50, -135);
            dt.drive(-800, 1500, 80, -135);
            bot::motors::lower.spin(vex::forward, 8.0, vex::volt);
            bot::motors::upper.spin(vex::reverse, 100, vex::percent);
            dt.brake();
            dt.drive(-200, 800, 50, -135);
            vex::task mid_heading_adjust_task = vex::task([]() -> int {
                dt.turn_to_heading(-135, 1000, 100);
                return 0;
            });
            vex::task::sleep(2000);
            dt.drive(-100, 500, 30, -135);
            bot::motors::intake.stop();
            bot::pistons::match_load_piston.set(false);
            dt.drive(250, 1000, 50, -135);
            dt.drive(500, 1000, 100, -135);
            dt.drive(300, 1000, 100, -90);
            dt.drive(350, 1000, 80, 30);
            dt.turn_to_heading(170, 800, 80);
            bot::pistons::arm_piston.set(false);
            dt.drive(-650, 1500, 60, 180);
            bot::motors::right_dt.spin(vex::forward, 30, vex::percent);
            bot::motors::left_dt.spin(vex::reverse, 20, vex::percent);
            dt.brake();
            bot::Controller1.Screen.setCursor(2,1);
            double end_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.print("end time: %.1f", end_time);
            bot::Controller1.Screen.setCursor(3,1);
            bot::Controller1.Screen.print("time taken: %.1f", end_time - start_time);
            vex::task::sleep(15000);
            return;*/

        }

        void left_6_3() {
            bot::sensors::imu.setHeading(0.0, vex::degrees);
            double start_time = vex::timer::system();
            bot::Controller1.Screen.clearScreen();
            bot::pistons::arm_pistons.set(true);
            bot::pistons::hood_piston.set(false);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(500, 1500, 80, -25);
            bot::pistons::_match_load_piston.set(true);
            dt.drive(150, 500, 60, -25);
            vex::task match_load_task = vex::task([]() -> int {
                vex::task::sleep(200);
                bot::pistons::_match_load_piston.set(false);
                return 0;
            });
            dt.drive(250, 1500, 80, -60);
            dt.drive(350, 1000, 80, -80);
            bot::pistons::_match_load_piston.set(true);
            vex::task match_load_task2 = vex::task([]() -> int {
                vex::task::sleep(500);
                bot::pistons::_match_load_piston.set(true);
                return 0;
            });
            dt.drive_for(250, 600, 80, -80);
            vex::task match_load_task3 = vex::task([]() -> int {
                vex::task::sleep(800);
                bot::pistons::_match_load_piston.set(false);
                return 0;
            });
            dt.drive(-650, 2000, 80, 0);
            dt.drive(-300, 1500, 80, 90);
            dt.drive_dist(540, 1000, 70, 90, 5.0, bot::sensors::back_dist, bot::rev);
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            bot::Controller1.Screen.print("back dist: %.1f", bot::sensors::back_dist.objectDistance(vex::mm));
            printf("back dist: %.1f", bot::sensors::back_dist.objectDistance(vex::mm));
            dt.drive(-500, 800, 50, 180);
            bot::pistons::_hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            vex::task heading_adjust_task = vex::task([]() -> int {
                dt.drive(-500, 500, 40, 180);
                dt.drive(25, 500, 50, 180);
                dt.turn_to_heading(180, 500, 100);
                dt.drive(-400, 500, 40, 180);
                return 0;
            });
            dt.brake();
            vex::task::sleep(1500);
            dt.coast();
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::motors::upper.spin(vex::reverse, 8.0, vex::volt);

            vex::task upper_task = vex::task([]() -> int {
            while (bot::motors::upper.torque(vex::torqueUnits::Nm) < 0.3) {
                vex::task::sleep(5);
            } bot::motors::upper.stop(); return 0;});

            dt.drive(200, 800, 30, 180);
            bot::pistons::_match_load_piston.set(true);
            bot::pistons::_hood_piston.set(false);
            dt.drive(400, 1000, 50, 180);
            dt.drive(200, 500, 35, 180);
            vex::task::sleep(100);
            dt.drive(300, 500, 30, 180);
            dt.coast();
            dt.drive(-150, 1000, 50, -135);
            dt.turn_to_heading(-135, 3000, 80);
            bot::pistons::_match_load_piston.set(false);
            dt.drive(-1100, 2500, 80, -135);
            vex::task lower_task = vex::task([]() -> int {
                vex::task::sleep(300);
                bot::pistons::mid_piston.set(true);
                bot::motors::lower.spin(vex::forward, 100, vex::percent);
                bot::motors::upper.spin(vex::reverse, 100, vex::percent);
                return 0;
            });
            dt.drive(-400, 500, 40, -135);
            dt.brake();
            vex::task::sleep(1000);
            dt.drive(150, 500, 50, -135);
            dt.brake();
            vex::task::sleep(100);
            bot::motors::upper.stop();
            bot::pistons::mid_piston.set(false);
            dt.drive(-150, 500, 70, -135);
            vex::task::sleep(200);
            dt.drive(200, 600, 40, -125);
            dt.drive(400, 1500, 80, -125);
            dt.turn_to_heading(180, 600, 80);
            bot::pistons::arm_pistons.set(false);
            dt.drive(-600, 1000, 80, 180);
            dt.hold();
            double end_time2 = vex::timer::system();
            bot::motors::left_dt.spin(vex::forward, 3.0, vex::volt);
            bot::motors::right_dt.spin(vex::reverse, 100, vex::percent);
            vex::task::sleep(200);
            bot::motors::all_dt.stop();
            dt.hold();
            bot::Controller1.Screen.setCursor(2,1);
            double end_time = vex::timer::system();
            bot::Controller1.Screen.print("time taken: %.1f", end_time - start_time);
            vex::task::sleep(15000);
            return;
        }

        void left_6() {
            bot::sensors::imu.setHeading(0.0, vex::degrees);
            double start_time = vex::timer::system();
            bot::Controller1.Screen.clearScreen();
            bot::pistons::arm_pistons.set(true);
            bot::pistons::hood_piston.set(false);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(650, 1500, 65, -25);
            bot::pistons::match_load_piston.set(true);
            vex::task match_load_task = vex::task([]() -> int {
                vex::task::sleep(300);
                bot::pistons::match_load_piston.set(false);
                return 0;
            });
            dt.drive(250, 1500, 80, -65);
            dt.drive(250, 1000, 80, -80);
            vex::task match_load_task2 = vex::task([]() -> int {
                vex::task::sleep(500);
                bot::pistons::match_load_piston.set(true);
                return 0;
            });
            dt.drive_for(500, 1000, 80, -80);
            vex::task match_load_task3 = vex::task([]() -> int {
                vex::task::sleep(800);
                bot::pistons::match_load_piston.set(false);
                return 0;
            });
            dt.drive(-750, 2000, 80, 0);
            dt.drive(-300, 1500, 80, 90);
            dt.drive_dist(590, 1000, 50, 90, 5.0, bot::sensors::back_dist, bot::rev);
            bot::pistons::hood_piston.set(true);
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            bot::Controller1.Screen.print("back dist: %.1f", bot::sensors::back_dist.objectDistance(vex::mm));
            printf("back dist: %.1f", bot::sensors::back_dist.objectDistance(vex::mm));
            dt.drive(-500, 800, 50, 180);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            vex::task heading_adjust_task = vex::task([]() -> int {
                dt.drive(-500, 500, 80, 180);
                dt.drive(25, 500, 50, 180);
                dt.turn_to_heading(180, 500, 100);
                dt.drive(-400, 500, 50, 180);
                return 0;
            });
            dt.brake();
            vex::task::sleep(1500);
            dt.coast();
            bot::motors::intake.stop();
            dt.drive(300, 1500, 50, 90);
            dt.turn_to_heading(180, 1000, 60);
            bot::pistons::arm_pistons.set(false);
            dt.drive(-750, 1500, 60, 180);
            dt.hold();
        }

        void right_6() {
            bot::sensors::imu.setHeading(0.0, vex::degrees);
            double start_time = vex::timer::system();
            bot::Controller1.Screen.clearScreen();
            bot::pistons::arm_pistons.set(true);
            bot::pistons::hood_piston.set(false);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(650, 1500, 65, 25);
            bot::pistons::match_load_piston.set(true);
            vex::task match_load_task = vex::task([]() -> int {
                vex::task::sleep(300);
                bot::pistons::match_load_piston.set(false);
                return 0;
            });
            dt.drive(250, 1500, 80, 65);
            dt.drive(250, 1000, 80, 80);
            vex::task match_load_task2 = vex::task([]() -> int {
                vex::task::sleep(500);
                bot::pistons::match_load_piston.set(true);
                return 0;
            });
            dt.drive_for(500, 1000, 80, 80);
            vex::task match_load_task3 = vex::task([]() -> int {
                vex::task::sleep(800);
                bot::pistons::match_load_piston.set(false);
                return 0;
            });
            dt.drive(-750, 2000, 80, 0);
            dt.drive(-300, 1500, 80, -90);
            dt.drive_dist(590, 1000, 50, -90, 5.0, bot::sensors::back_dist, bot::rev);
            bot::pistons::hood_piston.set(true);
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            bot::Controller1.Screen.print("back dist: %.1f", bot::sensors::back_dist.objectDistance(vex::mm));
            printf("back dist: %.1f", bot::sensors::back_dist.objectDistance(vex::mm));
            dt.drive(-500, 800, 50, 180);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            vex::task heading_adjust_task = vex::task([]() -> int {
                dt.drive(-500, 500, 80, 180);
                dt.drive(25, 500, 50, 180);
                dt.turn_to_heading(180, 500, 100);
                dt.drive(-400, 500, 50, 180);
                return 0;
            });
            dt.brake();
            vex::task::sleep(1500);
            dt.coast();
            bot::motors::intake.stop();
            dt.drive(300, 1500, 50, 90);
            dt.turn_to_heading(180, 1000, 60);
            bot::pistons::arm_pistons.set(false);
            dt.drive(-750, 1500, 60, 180);
            dt.hold();
        }

        void right_7() {
            double start_time = vex::timer::system();
            bot::sensors::imu.setHeading(0.0, vex::degrees);
            bot::pistons::arm_pistons.set(true);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(450, 1500, 80, 25);
            bot::pistons::match_load_piston.set(true);
            dt.drive(200, 1000, 50, 25);
            bot::motors::upper.spin(vex::forward, 6.0, vex::volt);
            dt.drive(-50, 200, 30, -45);
            dt.drive(-500, 1500, 80, -45);
            dt.drive(-200, 1500, 80, -90);
            bot::motors::upper.stop();
            dt.drive_dist(470, 1000, 60, -90, 5.0, bot::sensors::back_dist, bot::rev);
            dt.turn_to_heading(180, 1000, 60);
            dt.drive(800, 1000, 40, 180);
            dt.drive(-650, 1000, 70, 180);
            bot::pistons::_match_load_piston.set(false);
            bot::pistons::_hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-500, 500, 40, 180);
            vex::task::sleep(1000);
            dt.drive(350, 1000, 80, 90);
            bot::motors::intake.stop();
            bot::pistons::_hood_piston.set(false);
            dt.turn_to_heading(180, 500, 100);
            bot::pistons::arm_pistons.set(false);
            dt.drive(-550, 1000, 80, 180);
            dt.drive(-200, 500, 40, 180);
            bot::motors::right_dt.spin(vex::reverse, 6.0, vex::volt);
            bot::motors::left_dt.spin(vex::forward, 6.0, vex::volt);
            double end_time = vex::timer::system();
            vex::task::sleep(200);
            bot::motors::all_dt.stop();
            dt.hold();
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            bot::Controller1.Screen.print("time taken: %.1f", end_time - start_time);
            vex::task::sleep(15000);
            /*            dt.drive(650, 1500, 80, 20);
            bot::pistons::match_load_piston.set(true);
            dt.drive(-300, 1000, 70, -70);
            bot::pistons::match_load_piston.set(false);
            dt.drive_dist(500, 1000, 50, -90, 5.0, bot::sensors::back_dist, bot::rev);
            dt.turn_to_heading(180, 1000, 60);
            bot::pistons::match_load_piston.set(true);
            vex::task::sleep(300);
            dt.drive(1000, 1000, 50, 180);
            dt.drive(-200, 1000, 20, 180);
            bot::pistons::match_load_piston.set(false);
            dt.drive(-450, 1000, 60, 180);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.brake();
            dt.drive(-500, 500, 50, 180);
            vex::task::sleep(1000);
            dt.drive(50, 300, 50, 180);
            dt.drive(200, 800, 50, 90);
            dt.drive_dist(200, 1000, 50, 90, 3.0, bot::sensors::front_dist, bot::fwd);
            bot::pistons::arm_pistons.set(false);
            dt.drive(500, 1500, 60, 180);
            dt.brake();
            return;
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            double start_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.print("start time: %.1f", start_time);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::pistons::arm_pistons.set(true);
            dt.drive(650, 1500, 100, 35);
            bot::pistons::match_load_piston.set(true);
            dt.drive(300, 1000, 50, 150);
            dt.drive(600, 1500, 70, 135);
            dt.drive(1000, 1500, 50, 180);
            dt.drive(-200, 800, 30, 180);
            dt.drive(-450, 1000, 60, 180);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            bot::pistons::match_load_piston.set(false);
            dt.drive(-200, 500, 40, 180);
            dt.brake();
            vex::task::sleep(500);
            dt.drive(-200, 500, 70, 180);
            dt.coast();
            vex::task::sleep(200);
            dt.drive(170, 1500, 50, -90);
            bot::pistons::arm_pistons.set(false);
            dt.turn_to_heading(180, 500, 100);
            dt.drive(-500, 1500, 60, 180);
            dt.brake();
            bot::Controller1.Screen.setCursor(2,1);
            double end_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.print("end time: %.1f", end_time);
            bot::Controller1.Screen.setCursor(3,1);
            bot::Controller1.Screen.print("time taken: %.1f", end_time - start_time);
            return;*/
        }

        void right_4_3() {
            bot::pistons::arm_pistons.set(false);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::pistons::match_load_piston.set(true);
            dt.drive(450, 1000, 100, 0);
            dt.drive(700, 1400, 50, 90);
            dt.drive(-200, 800, 20, 90);
            dt.drive(-500, 1000, 50, 90);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            bot::pistons::match_load_piston.set(false);
            dt.brake();
            vex::task heading_adjust_task = vex::task([]() -> int {
                dt.turn_to_heading(90, 700, 100);
                dt.drive(-300, 700, 50, 90);
                return 0;
            });
            vex::task::sleep(1500);
            bot::motors::intake.stop();
            dt.coast();
            dt.drive(200, 1000, 30, 90);
            dt.drive_for(420, 1200, 60, 90);
            dt.turn_to_heading(-135, 1000, 50);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(1000, 1500, 60, -135);
            dt.drive(350, 1000, 50, -135);
            dt.brake();
            vex::task::sleep(300);
            bot::motors::lower.spin(vex::reverse, 100, vex::percent);
            vex::task::sleep(1500);
            dt.coast();
            dt.drive(-300, 700, 60, -135);
            dt.drive(-230, 700, 60, 180);
            dt.drive(-300, 700, 60, 90);
            dt.drive(-400, 700, 50, 90);
            dt.brake();
        }

        void left_4() {
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            double start_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.print("start time: %.1f", start_time);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::pistons::arm_pistons.set(true);
            dt.drive(650, 1500, 100, -35);
            bot::pistons::match_load_piston.set(true);
            dt.drive(-400, 800, 100, 90);
            dt.drive(-700, 800, 100, -135);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            bot::pistons::match_load_piston.set(false);
            dt.turn_to_heading(180, 500, 100);
            dt.drive(-400, 500, 70, 180);
            dt.drive(170, 1500, 60, -90);
            dt.turn_to_heading(180, 500, 100);
            bot::pistons::arm_pistons.set(false);
            dt.drive(-500, 1000, 60, 180);
            dt.brake();
            bot::Controller1.Screen.setCursor(2,1);
            double end_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.print("end time: %.1f", end_time);
            bot::Controller1.Screen.setCursor(3,1);
            bot::Controller1.Screen.print("time taken: %.1f", end_time - start_time);
        }

        void right_4() {
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            double start_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.print("start time: %.1f", start_time);
            bot::pistons::arm_pistons.set(true);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(650, 1500, 100, 35);
            bot::pistons::match_load_piston.set(true);
            dt.drive(-400, 800, 100, -90);
            dt.drive(-700, 700, 100, -135);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            bot::pistons::match_load_piston.set(false);
            dt.turn_to_heading(180, 500, 100);
            dt.drive(-400, 500, 70, 180);
            dt.drive(170, 1500, 60, -90);
            bot::motors::intake.stop();
            dt.turn_to_heading(-180, 500, 100);
            bot::pistons::arm_pistons.set(false);
            dt.drive(-500, 1000, 60, 180);
            dt.brake();
            bot::Controller1.Screen.setCursor(2,1);
            double end_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.print("end time: %.1f", end_time);
            bot::Controller1.Screen.setCursor(3,1);
            bot::Controller1.Screen.print("time taken: %.1f", end_time - start_time);
        }

        void sawp() {
            bot::sensors::imu.setHeading(270.0, vex::degrees);
            double start_time = vex::timer::system();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::pistons::arm_pistons.set(true);
            bot::pistons::hood_piston.set(false);
            dt.drive(-450, 1000, 80, -90);
            bot::pistons::match_load_piston.set(true);
            dt.drive_dist(480, 1000, 60, -90, 5.0, bot::sensors::back_dist, bot::rev);
            dt.turn_to_heading(180, 1000, 70);
            dt.drive(600, 500, 40, 180);
            vex::task::sleep(100);
            dt.drive(200, 200, 60, 180);
            dt.drive(-650, 1000, 70, 180);
            bot::pistons::match_load_piston.set(false);
            bot::pistons::hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-400, 500, 50, 180);
            vex::task::sleep(600);
            bot::motors::intake.stop();
            bot::pistons::hood_piston.set(false);
            while(bot::sensors::imu.heading(vex::degrees) < 270) {
                bot::motors::right_dt.spin(vex::reverse, 50, vex::percent);
                bot::motors::left_dt.spin(vex::forward, 100, vex::percent);
                vex::task::sleep(5);
            }
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::motors::upper.spin(vex::reverse, 100, vex::percent);
            dt.coast();
            dt.drive(200, 800, 40, -90);
            dt.turn_to_heading(-90, 500, 100);
            bot::motors::upper.stop();
            dt.drive(1050, 3000, 80, -90);
            bot::pistons::match_load_piston.set(true);
            dt.drive(200, 1000, 60, -90);
            bot::pistons::match_load_piston.set(false);
            bot::motors::lower.spin(vex::forward, 5.0, vex::volt);
            bot::motors::upper.spin(vex::reverse, 6.0, vex::volt);
            dt.turn_to_heading(-135, 1000, 80);
            dt.drive(-400, 1000, 60, -135);
            bot::pistons::mid_piston.set(true);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::motors::upper.spin(vex::reverse, 100, vex::percent);
            vex::task mid_task = vex::task([]() -> int {
                dt.drive(-200, 500, 25, -135);
                return 0;
            });
            vex::task::sleep(1000);
            bot::motors::intake.stop();
            bot::pistons::mid_piston.set(false);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::motors::upper.spin(vex::forward, 6.0, vex::volt);

            vex::task upper_task = vex::task([]() -> int {
            while (bot::motors::upper.torque(vex::torqueUnits::Nm) < 0.3) {
                vex::task::sleep(5);
            } bot::motors::upper.stop(); return 0;});

            dt.drive(900, 2000, 80, -150);
            dt.drive(400, 1000, 80, -90);

            dt.drive_dist(365, 1000, 60, -90, 5.0, bot::sensors::front_dist, bot::fwd);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::pistons::match_load_piston.set(true);
            dt.turn_to_heading(180, 1000, 100);
            dt.drive(600, 800, 40, 180);
            dt.drive(-650, 1000, 70, 180);
            bot::pistons::match_load_piston.set(false);
            bot::pistons::hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-400, 500, 50, 180);
            dt.brake();
            double end_time = vex::timer::system();
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            bot::Controller1.Screen.print("time taken: %.1f", end_time - start_time);
            vex::task::sleep(15000);
            return;
            
            /*
            double start_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            bot::Controller1.Screen.print("start time: %.1f", start_time);
            bot::pistons::arm_piston.set(true);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(150, 500, 30, 0);
            dt.drive(-300, 500, 80, 0);
            dt.drive(-600, 1500, 100, 0);
            dt.drive(-300, 800, 50, 0);
            dt.drive_for(-200, 500, 100, 0);
            bot::pistons::match_load_piston.set(true);
            dt.turn_to_heading(-90, 500, 100);
            dt.drive(500, 800, 50, -90);
            dt.drive(-200, 700, 35, -90);
            dt.drive(-470, 1000, 60, -90);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            bot::pistons::match_load_piston.set(false);
            dt.brake();
            vex::task::sleep(700);
            dt.drive(-100, 300, 40, -90);
            dt.coast();
            dt.drive(125, 700, 40, -90);
            dt.drive(530, 1500, 50, 45);
            dt.drive(400, 800, 60, 0);
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(900, 2000, 100, 0);
            dt.drive(400, 1000, 70, 0);
            bot::pistons::match_load_piston.set(true);
            dt.drive_for(200, 400, 40, 0);
            vex::task mid_scoring_task = vex::task([]() -> int {
                vex::task::sleep(150);
                bot::motors::lower.spin(vex::reverse, 4.0, vex::volt);
                vex::task::sleep(500);
                bot::motors::lower.spin(vex::forward, 8.0, vex::volt);
                bot::motors::upper.spin(vex::reverse, 100, vex::percent);
                return 0;
            });
            dt.drive(-700, 1000, 50, -45);
            dt.brake();
            vex::task mid_task = vex::task([]() -> int {
                dt.turn_to_heading(-45, 1000, 50);
                return 0;
            });
            vex::task::sleep(300);
            dt.drive(-500, 500, 20, -45);
            vex::task::sleep(700);
            bot::motors::upper.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.coast();
            dt.drive(200, 500, 40, -45);
            dt.drive(700, 1500, 70, -45);
            dt.drive(200, 800, 50, -45);
            bot::pistons::match_load_piston.set(true);
            dt.drive(850, 1200, 50, -90);
            double right_distance = bot::sensors::right_dist.objectDistance(vex::mm);
            double heading_correct;
            if (right_distance == 9999 || right_distance <= 0) heading_correct = 0.0;
            else heading_correct = (500 - right_distance) * 0.10;
            dt.drive(-200, 700, 35, -90 - heading_correct);
            dt.drive(-500, 1000, 60, -90);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.brake();
            vex::task::sleep(15000);
            bot::Controller1.Screen.setCursor(2,1);
            double end_time = bot::Brain.Timer.time(vex::msec);
            bot::Controller1.Screen.print("end time: %.1f", end_time);
            bot::Controller1.Screen.setCursor(3,1);
            bot::Controller1.Screen.print("time taken: %.1f", end_time - start_time);*/
        }

        void counter_sawp(){
            bot::sensors::imu.setHeading(270.0, vex::degrees);
            double start_time = vex::timer::system();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::pistons::arm_pistons.set(true);
            bot::pistons::hood_piston.set(false);
            dt.drive(-450, 1000, 80, -90);
            bot::pistons::match_load_piston.set(true);
            dt.drive_dist(480, 800, 60, -90, 5.0, bot::sensors::back_dist, bot::rev);
            dt.turn_to_heading(180, 600, 60);
            dt.drive(600, 500, 40, 180);
            vex::task::sleep(100);
            dt.drive(200, 200, 60, 180);
            dt.drive(-650, 1000, 70, 180);
            bot::pistons::_match_load_piston.set(false);
            bot::pistons::_hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-400, 500, 50, 180);
            vex::task::sleep(600);
            bot::motors::intake.stop();
            bot::pistons::_hood_piston.set(false);
            while(bot::sensors::imu.heading(vex::degrees) < 270) {
                bot::motors::right_dt.spin(vex::reverse, 50, vex::percent);
                bot::motors::left_dt.spin(vex::forward, 100, vex::percent);
                vex::task::sleep(5);
            }
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::motors::upper.spin(vex::reverse, 100, vex::percent);
            dt.coast();
            dt.drive(200, 800, 40, -90);
            dt.turn_to_heading(-90, 500, 100);
            bot::motors::upper.stop();
            dt.drive(1000, 3000, 80, -90);
            bot::pistons::match_load_piston.set(true);
            dt.drive(200, 1000, 80, -90);
            bot::motors::lower.spin(vex::forward, 100.0, vex::percent);
            dt.drive(700, 1500, 80, -150);
            bot::pistons::_match_load_piston.set(false);
            dt.drive(100, 500, 50, -90);
            dt.drive_dist(365, 700, 60, -90, 5.0, bot::sensors::front_dist, bot::fwd);
            dt.turn_to_heading(180, 600, 60);
            dt.drive(-500, 400, 50, 180);
            vex::task heading_adjust_task = vex::task([]() -> int {
                dt.drive(-500, 500, 80, 180);
                dt.drive(25, 500, 50, 180);
                dt.turn_to_heading(180, 500, 100);
                dt.drive(-400, 500, 50, 180);
                return 0;
            });
            bot::pistons::_hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            vex::task::sleep(800);
            heading_adjust_task.stop();
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::motors::upper.spin(vex::forward, 8.0, vex::volt);

            /*vex::task upper_task = vex::task([]() -> int {
            while (bot::motors::upper.torque(vex::torqueUnits::Nm) < 0.35) {
                vex::task::sleep(5);
            } bot::motors::upper.stop(); return 0;});*/

            dt.drive(200, 800, 40, 180);
            bot::pistons::_hood_piston.set(false);
            bot::pistons::_match_load_piston.set(true);
            dt.drive(400, 1000, 50, 180);
            dt.drive(200, 500, 35, 180);
            vex::task::sleep(100);
            dt.drive(300, 500, 30, 180);
            dt.coast();
            dt.drive(-150, 1000, 50, -135);
            dt.turn_to_heading(-135, 3000, 80);
            bot::pistons::_match_load_piston.set(false);
            dt.drive(-1100, 2500, 80, -135);
            vex::task lower_task = vex::task([]() -> int {
                vex::task::sleep(300);
                bot::pistons::mid_piston.set(true);
                bot::motors::lower.spin(vex::forward, 100, vex::percent);
                bot::motors::upper.spin(vex::reverse, 100, vex::percent);
                return 0;
            });
            dt.drive(-400, 500, 40, -135);
            dt.brake();
            double end_time = vex::timer::system();
            bot::Controller1.Screen.clearScreen();
            bot::Controller1.Screen.setCursor(1,1);
            bot::Controller1.Screen.print("time taken: %.1f", end_time - start_time);
            vex::task::sleep(15000);
            return;   
        }

        void skills() {
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::pistons::arm_pistons.set(true);
            dt.drive(1000, 2000, 40, 0);
            dt.drive(800, 1500, 70, 0);
            dt.brake();
            dt.turn_to_heading(90, 1000, 100);
            dt.coast();
            dt.drive(-300, 800, 40, 90);
            double left_distance = bot::sensors::left_dist.objectDistance(vex::mm);
            double heading_correct;
            if (left_distance == 9999 || left_distance <= 0) heading_correct = 0.0;
            else if (left_distance < 300) heading_correct = (400 - left_distance) * 0.07;
            else heading_correct = (850 - left_distance) * 0.07;
            vex::task::sleep(50);
            dt.drive(650, 1500, 60, 100+heading_correct);
            dt.drive_for(450 - (10.0*heading_correct), 1000, 50, 100+heading_correct);
            dt.turn_to_heading(-45, 1200, 100);
            bot::motors::lower.stop();
            bot::motors::intake.spin(vex::reverse, 5.0, vex::volt);
            dt.drive(-450 - (20.0*heading_correct), 1000, 60, -45);
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 11.0, vex::volt);
            bot::motors::upper.spin(vex::reverse, 100, vex::percent);
            dt.brake();
            vex::task mid_task = vex::task([]() -> int {
                dt.turn_to_heading(-45, 1500, 100);
                return 0;
            });
            vex::task::sleep(1500);
            bot::motors::lower.spin(vex::forward, 11.0, vex::volt);
            vex::task::sleep(1000);
            bot::motors::upper.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(-100, 500, 30, -45);
            dt.drive(300, 1000, 50, -45);
            bot::pistons::match_load_piston.set(true);
            dt.drive(750, 1500, 70, -45);
            dt.drive(700, 1500, 50, -90);
            dt.drive(500, 1200, 40, -90);
            dt.turn_to_heading(-90, 500, 100);
            double right_distance = bot::sensors::right_dist.objectDistance(vex::mm);
            double heading_correct_2;
            if (right_distance == 9999 || right_distance <= 0) heading_correct_2 = 0.0;
            else heading_correct_2 = (600 - right_distance);
            dt.drive(-250 - sqrt(2.0)*heading_correct_2, 1500, 40, -45);
            dt.drive(-350, 1000, 40, -45);
            bot::pistons::match_load_piston.set(false);
            dt.drive(-200, 1000, 40, -45);
            dt.drive(-400, 1500, 80, -45);
            dt.drive_for(-200, 500, 50, -45);
            bot::motors::lower.spin(vex::reverse, 8.0, vex::volt);
            dt.drive(-300, 500, 35, -45);
            bot::motors::lower.spin(vex::forward, 10.0, vex::volt);
            bot::motors::upper.spin(vex::reverse, 5.5, vex::volt);
            mid_task = vex::task([]() -> int {
                dt.turn_to_heading(-45, 1000, 100);
                return 0;
            });
            vex::task::sleep(1500);
            bot::motors::upper.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(150, 800, 30, -45);
            dt.drive(500, 1500, 50, 85);
            dt.drive(650, 1500, 60, 80);
            double left_distance_2 = bot::sensors::left_dist.objectDistance(vex::mm);
            double dsr_distance;
            if (left_distance_2 == 9999 || left_distance_2 <= 0) dsr_distance = 350.0;
            else if (left_distance_2 < 700.0) dsr_distance = left_distance_2 - 200.0;
            else dsr_distance = left_distance_2 - 650.0;
            bot::pistons::match_load_piston.set(true);
            dt.drive(400, 1500, 50, 20);
            dt.drive(dsr_distance, 1200, 60, 20);
            dt.drive(200, 1000, 30, 90);
            dt.drive(-400, 1000, 40, 90);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.brake();
            vex::task heading_adjust_task = vex::task([]() -> int {
                dt.drive(20, 200, 25, 90);
                dt.brake();
                dt.turn_to_heading(90, 1000, 100);
                dt.drive(-100, 500, 50, 90);
                return 0;
            });
            vex::task::sleep(1500);
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(200, 800, 20, 90);
            dt.drive(450, 800, 50, 90);
            dt.drive(700, 1700, 40, 90);
            dt.drive(-300, 1000, 30, 90);
            dt.drive(-400, 1000, 70, 90);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.brake();
            heading_adjust_task = vex::task([]() -> int {
                dt.brake();
                dt.turn_to_heading(90, 1500, 100);
                dt.drive(-500, 500, 40, 90);
                return 0;
            });
            vex::task::sleep(2000);
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.coast();
            bot::pistons::match_load_piston.set(false);
            dt.drive(600, 800, 80, 135);
            dt.drive(500, 800, 100, 180);
            dt.turn_to_heading(180, 500, 100);
            bot::sensors::imu.setHeading(180, vex::degrees);
            dt.coast();
            dt.drive(1000, 2000, 40, 180);
            dt.drive(400, 1500, 100, 180);
            dt.drive(600, 1500, 60, -120);
            dt.drive(1500, 1200, 60, 180);
            dt.drive(-200, 800, 50, 135);
            vex::task intake_task = vex::task([]() -> int {
                vex::task::sleep(1000);
                bot::motors::intake.spin(vex::forward, 100, vex::percent);
                return 0;
            });
            dt.drive(-300, 600, 40, 90);
            dt.drive(-1000, 1000, 50, 90);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.brake();
            bot::pistons::match_load_piston.set(true);
            vex::task::sleep(500);
            /*
            heading_adjust_task = vex::task([]() -> int {
                dt.turn_to_heading(90, 500, 100);
                dt.drive(-500, 500, 40, 90);
                return 0;
            });
            vex::task::sleep(1000);
            */
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.coast();
            dt.drive(200, 800, 20, 90);
            dt.drive(350, 1000, 50, 90);
            dt.drive(600, 2000, 50, 90);
            dt.drive(-370, 1000, 50, 45);
            dt.drive(-300, 1000, 50, 90);
            bot::pistons::match_load_piston.set(false);
            dt.drive(-1700, 3000, 60, 85);
            dt.drive(-400, 1500, 60, 180);
            dt.drive(-300, 1200, 30, -90);
            intake_task = vex::task([]() -> int {
                vex::task::sleep(500);
                bot::motors::intake.spin(vex::forward, 100, vex::percent);
                return 0;
            });
            dt.drive(-600, 1000, 40, -90);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.brake();
            heading_adjust_task = vex::task([]() -> int {
                dt.drive(20, 200, 25, -90);
                dt.turn_to_heading(-90, 1000, 100);
                bot::pistons::match_load_piston.set(true);
                dt.drive(-500, 800, 40, -90);
                return 0;
            });
            vex::task::sleep(2000);
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.coast();
            dt.drive(200, 800, 20, -90);
            dt.drive(350, 1000, 50, -90);
            dt.drive(600, 1700, 50, -90);
            dt.drive(-300, 1000, 30, -90);
            dt.drive(-400, 1000, 70, -90);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.brake();
            dt.drive(-500, 500, 40, -90);
            heading_adjust_task = vex::task([]() -> int {
                dt.drive(20, 200, 25, -90);
                dt.turn_to_heading(-90, 1500, 100);
                bot::pistons::match_load_piston.set(false);
                return 0;
            });
            vex::task::sleep(2200);
            dt.drive(25, 400, 100, -90);
            dt.drive(300, 1000, 80, -30);
            dt.drive(300, 1000, 80, -45);
            dt.drive(500, 1000, 100, 0);
            dt.drive(800, 2000, 50, 0);
            dt.brake();
        }

        void skills_80(){
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::sensors::imu.setHeading(90, vex::degrees);
            bot::pistons::arm_pistons.set(true);
            dt.drive_for(850, 1500, 60, 90);
            dt.turn_to_heading(180, 1000, 50);
            bot::pistons::match_load_piston.set(true);
            vex::task::sleep(500);
            dt.drive(1000, 2000, 50, 180);
            dt.drive(-400, 1000, 50, -135);
            dt.drive(-300, 1000, 40, 180);
            bot::pistons::match_load_piston.set(false);
            bot::motors::lower.stop();
            dt.drive(-700, 1500, 70, 180);
            dt.drive(-300, 1000, 60, 180);
            dt.drive(-700, 1500, 60, 180);
            dt.drive(-380, 1000, 50, 90);
            dt.drive(-400, 1000, 40, 0);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-500, 800, 40, 0);
            dt.brake();
            vex::task::sleep(1500);
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.coast();
            dt.drive(200, 800, 30, 0);
            bot::pistons::match_load_piston.set(true);
            dt.drive(1000, 2000, 50, 0);
            dt.drive(-200, 800, 30, 0);
            dt.drive(-450, 1000, 50, 0);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-500, 500, 40, 0);
            dt.brake();
            vex::task::sleep(1500);
            dt.coast();
            bot::motors::intake.stop();
            bot::pistons::match_load_piston.set(false);
            dt.drive(600, 800, 80, -45);
            dt.drive(500, 800, 100, -90);
            dt.turn_to_heading(-90, 500, 100);
            dt.brake();
            vex::task::sleep(200);
            dt.coast();
            bot::sensors::imu.setHeading(270, vex::degrees);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(1200, 2000, 40, -90);
            dt.drive(400, 1500, 60, -90);
            dt.drive(600, 1500, 60, -160);
            dt.drive(2000, 1500, 60, -90);
            dt.drive(-200, 800, 50, -45);
            dt.drive(-300, 600, 40, 0);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-500, 800, 50, 0);
            dt.drive(25, 500, 30, 0);
            dt.turn_to_heading(0, 500, 100);
            dt.drive(-500, 800, 40, 0);
            dt.brake();
            vex::task::sleep(500);
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.coast();
            bot::pistons::match_load_piston.set(true);
            dt.drive(200, 800, 30, 0);
            dt.drive(1000, 2000, 50, 0);
            dt.drive(-400, 1000, 50, 45);
            dt.drive(-300, 1000, 40, 0);
            bot::pistons::match_load_piston.set(false);
            bot::motors::lower.stop();
            dt.drive(-700, 1500, 70, 0);
            dt.drive(-300, 1000, 60, 0);
            dt.drive(-700, 1500, 60, 0);
            dt.drive(-380, 1000, 50, -90);
            dt.drive(-400, 1000, 40, 180);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-500, 800, 40, 180);
            dt.brake();
            vex::task::sleep(1500);
            bot::motors::intake.stop();
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.coast();
            bot::pistons::match_load_piston.set(true);
            dt.drive(200, 800, 30, 180);
            dt.drive(1000, 2000, 50, 180);
            dt.drive(-200, 800, 30, 180);
            dt.drive(-450, 1000, 50, 180);
            bot::pistons::match_load_piston.set(false);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-500, 800, 40, 180);
            dt.brake();
            vex::task::sleep(1500);
            bot::motors::intake.stop();
            dt.coast();
            dt.drive(600, 800, 80, 135);
            dt.drive(500, 800, 100, 90);
            dt.turn_to_heading(90, 500, 100);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(800, 2500, 40, 90);
            dt.drive(-150, 1000, 100, 90);
            dt.brake();


        }

        void skills_worlds(){
            bot::sensors::imu.setHeading(270.0, vex::degrees);
            bot::pistons::arm_pistons.set(true);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(-600, 1000, 80, -90);
            dt.drive_dist(480, 1000, 60, -90, 5.0, bot::sensors::back_dist, bot::rev);
            bot::pistons::match_load_piston.set(true);
            dt.turn_to_heading(180, 1000, 60);
            dt.drive(800, 1500, 30, 180);
            vex::task::sleep(200);
            dt.drive(600, 1000, 40, 180);
            bot::motors::upper.spin(vex::forward, 6.0, vex::volt);
            vex::task mid_task = vex::task([]() -> int {
                while (bot::motors::upper.torque(vex::torqueUnits::Nm) < 0.35) {
                    vex::task::sleep(5);
                } bot::motors::upper.stop(); return 0;});
            dt.drive(-550, 1000, 60, -135);
            dt.drive(-500, 1000, 60, 180);
            bot::pistons::match_load_piston.set(false);
            bot::motors::lower.stop();
            dt.drive(-1550, 3000, 80, 180);
            dt.turn_to_heading(90, 500, 80);
            dt.drive_dist(320, 1000, 50, 90, 5.0, bot::sensors::front_dist, bot::fwd);
            dt.drive(-500, 800, 50, 0);
            bot::pistons::hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-400, 500, 60, 0);
            vex::task heading_adjust_task = vex::task([]() -> int {
                dt.drive(25, 500, 50, 0);
                dt.turn_to_heading(0, 500, 50);
                dt.drive(-400, 500, 35, 0);
                return 0;
            });
            vex::task::sleep(2000);
            bot::motors::intake.stop();
            bot::pistons::hood_piston.set(false);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(250, 1000, 40, 0);
            bot::pistons::match_load_piston.set(true);
            dt.drive(300, 1000, 60, 0);
            dt.drive(600, 1500, 30, 0);
            vex::task::sleep(200);
            dt.drive(600, 1000, 40, 0);
            dt.drive(-300, 1000, 30, 0);
            dt.turn_to_heading(0, 1000, 50);
            dt.drive(-350, 1000, 60, 0);
            bot::pistons::match_load_piston.set(false);
            bot::pistons::hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-400, 500, 50, 0);
            dt.brake();
            vex::task::sleep(2000);
            bot::motors::intake.stop();
            bot::pistons::hood_piston.set(false);
            dt.drive(200, 1000, 50, 0);
            dt.turn_to_heading(90, 1000, 80);
            dt.drive(-2000, 3000, 80, 90);
            dt.drive_dist(485, 1500, 70, 90, 5.0, bot::sensors::back_dist, bot::rev);
            dt.turn_to_heading(0, 1000, 80);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::pistons::match_load_piston.set(true);
            dt.drive(1000, 1500, 30, 0);
            vex::task::sleep(200);
            dt.drive(600, 1000, 40, 0);
            dt.drive(-550, 1200, 60, 45);
            dt.drive(-500, 1000, 60, 0);
            bot::motors::lower.stop();
            bot::pistons::match_load_piston.set(false);
            dt.drive(-1550, 3000, 80, 0);
            dt.turn_to_heading(-90, 500, 80);
            dt.drive_dist(320, 1000, 50, -90, 5.0, bot::sensors::front_dist, bot::fwd);
            dt.drive(-500, 800, 50, 180);
            bot::pistons::hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-400, 500, 60, 180);
            heading_adjust_task = vex::task([]() -> int {
                dt.drive(25, 500, 50, 180);
                dt.turn_to_heading(180, 500, 50);
                dt.drive(-400, 500, 35, 180);
                return 0;
            });
            vex::task::sleep(2000);
            bot::motors::intake.stop();
            bot::pistons::hood_piston.set(false);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(250, 1000, 40, 180);
            bot::pistons::match_load_piston.set(true);
            dt.drive(300, 1000, 60, 180);
            dt.drive(600, 1500, 30, 180);
            vex::task::sleep(200);
            dt.drive(600, 1000, 40, 180);
            dt.drive(-300, 1000, 30, 180);
            dt.turn_to_heading(180, 500, 70);
            dt.drive(-350, 1000, 60, 180);
            bot::pistons::match_load_piston.set(false);
            bot::pistons::hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-400, 500, 50, 180);
            dt.brake();
            vex::task::sleep(2000);
            bot::motors::intake.stop();
            bot::pistons::hood_piston.set(false);
            dt.drive(800, 1500, 80, 135);
            dt.drive(400, 1000, 80, 90);
            dt.turn_to_heading(90, 500, 80);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(1000, 1500, 60, 90);
            dt.drive_dist(1600, 2000, 60, 90, 5.0, bot::sensors::front_dist, bot::fwd);
            vex::task::sleep(5000);
            bot::motors::intake.stop();
        
            return;


            dt.drive(600, 1500, 50, 0);
            for (int i = 0; i < 5; i++) {
                if (i%2) {bot::motors::left_dt.spin(vex::forward, 100, vex::percent); bot::motors::right_dt.spin(vex::reverse, 50, vex::percent);}
                else {bot::motors::left_dt.spin(vex::reverse, 50, vex::percent); bot::motors::right_dt.spin(vex::forward, 100, vex::percent);}
                vex::task::sleep(500);
            }
            dt.drive(500, 500, 80, 0);
            dt.turn_to_heading(0, 1000, 80);
            dt.drive(-450, 1000, 80, 0);
            dt.turn_to_heading(90, 1000, 80);
            bot::motors::lower.stop();
            dt.drive_dist(1050, 1000, 60, 90, 5.0, bot::sensors::front_dist, bot::fwd);
            dt.turn_to_heading(180, 1000, 80);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::motors::upper.spin(vex::reverse, 8.0, vex::volt);
            dt.drive_dist(1150, 1000, 60, 180, 5.0, bot::sensors::back_dist, bot::rev);
            dt.turn_to_heading(45, 1000, 80);
            dt.drive(-400, 1000, 60, 45);
            bot::pistons::mid_piston.set(true);
            bot::motors::upper.spin(vex::reverse, 60, vex::percent);
            vex::task::sleep(500);
            bot::motors::upper.spin(vex::reverse, 40, vex::percent);
            vex::task::sleep(500);
            bot::motors::upper.spin(vex::reverse, 30, vex::percent);
            vex::task::sleep(1000);
            dt.drive(200, 1000, 60, 45);
            dt.drive(500, 1000, 70, 0);
            dt.drive(400, 1500, 70, -90);
            dt.drive(1100, 2000, 80, -90);
            dt.drive_dist(350, 1000, 60, -90, 5.0, bot::sensors::front_dist, bot::fwd);
            dt.turn_to_heading(0, 1000, 80);
            

            return;



            /*bot::sensors::imu.setHeading(270, vex::degrees);
            bot::pistons::arm_pistons.set(true);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive_dist(500, 2000, 50, -90, 5.0, bot::sensors::back_dist, bot::rev);
            dt.turn_to_heading(180, 1000, 60);
            bot::pistons::match_load_piston.set(true);
            vex::task::sleep(500);
            dt.drive(600, 2500, 40, 180);
            dt.drive(-450, 1000, 60, -135);
            dt.drive(-350, 1000, 60, 180);
            bot::pistons::match_load_piston.set(false);
            bot::motors::lower.stop();
            dt.drive(-1800, 3000, 80, 180);
            dt.drive_for(-350, 800, 50, 180);
            dt.turn_to_heading(-90, 1000, 60);
            dt.drive_dist(590, 1000, 50, -90, 5.0, bot::sensors::back_dist, bot::rev);
            dt.drive(-500, 1000, 50, 0);
            bot::pistons::hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-400, 500, 60, 0);
            dt.brake();
            vex::task heading_adjust_task = vex::task([]() -> int {
                dt.drive(25, 500, 50, 0);
                dt.turn_to_heading(0, 500, 50);
                dt.drive(-400, 500, 60, 0);
                return 0;
            });
            vex::task::sleep(2000);
            bot::motors::intake.stop();
            bot::pistons::hood_piston.set(false);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(250, 1000, 20, 0);
            bot::pistons::match_load_piston.set(true);
            dt.drive(300, 1000, 50, 0);
            dt.drive(600, 1500, 30, 0);
            dt.drive(-300, 1000, 30, 0);
            dt.turn_to_heading(-5, 1000, 20);
            dt.drive(-400, 1000, 50, 0);
            bot::pistons::hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-400, 500, 50, 0);
            dt.brake();
            heading_adjust_task = vex::task([]() -> int {
                dt.drive(25, 500, 50, 0);
                dt.turn_to_heading(0, 500, 50);
                dt.drive(-400, 500, 60, 0);
                return 0;
            });
            vex::task::sleep(2000);
            bot::motors::intake.stop();
            bot::pistons::hood_piston.set(false);
            bot::pistons::match_load_piston.set(false);
            dt.drive(300, 1000, 50, 0);
            dt.brake();
            vex::task::sleep(200);
            dt.turn_to_heading(90, 1000, 60);
            dt.drive(-2000, 3000, 80, 90);
            dt.drive_dist(500, 1700, 60, 90, 5.0, bot::sensors::back_dist, bot::rev);
            dt.turn_to_heading(0, 1000, 60);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            bot::pistons::match_load_piston.set(true);
            vex::task::sleep(500);
            dt.drive(600,  2500, 40, 0);
            dt.drive(-450, 1000, 60, 45);
            dt.drive(-350, 1000, 60, 0);
            bot::pistons::match_load_piston.set(false);
            bot::motors::lower.stop();
            dt.drive(-1800, 3000, 80, 0);
            dt.drive_for(-350, 800, 50, 0);
            dt.turn_to_heading(90, 1000, 60);
            dt.drive_dist(590, 1000, 50, 90, 5.0, bot::sensors::back_dist, bot::rev);
            dt.drive(-500, 1000, 50, 180);
            bot::pistons::hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-400, 500, 60, 180);
            heading_adjust_task = vex::task([]() -> int {
                dt.drive(25, 500, 50, 180);
                dt.turn_to_heading(180, 500, 50);
                dt.drive(-400, 500, 60, 180);
                return 0;
            });
            vex::task::sleep(2000);
            bot::motors::intake.stop();
            bot::pistons::hood_piston.set(false);
            dt.drive(250, 1000, 20, 180);
            bot::pistons::match_load_piston.set(true);
            bot::motors::lower.spin(vex::forward, 100, vex::percent);
            dt.drive(300, 1000, 50, 180);
            dt.drive(600, 1500, 30, 180);
            dt.drive(-300, 1000, 30, 180);
            dt.turn_to_heading(175, 1000, 20);
            dt.drive(-400, 1000, 50, 180);
            bot::pistons::hood_piston.set(true);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.drive(-400, 500, 50, 180);
            dt.brake();
            bot::pistons::match_load_piston.set(false);
            heading_adjust_task = vex::task([]() -> int {
                dt.drive(25, 500, 50, 180);
                dt.turn_to_heading(180, 500, 50);
                dt.drive(-400, 500, 60, 180);
                return 0;
            });
            vex::task::sleep(2000);
            bot::motors::intake.stop();
            bot::pistons::hood_piston.set(true);
            dt.drive(800, 1500, 80, 130);
            dt.drive(400, 1000, 60, 110);
            bot::motors::intake.spin(vex::forward, 100, vex::percent);
            dt.brake();
            bot::pistons::match_load_piston.set(true);
            vex::task::sleep(200);
            dt.drive(1000, 2000, 60, 90);
            dt.drive(500, 1000, 50, 90);
            dt.brake();
            bot::pistons::match_load_piston.set(false);*/
        }

        void test() {
            dt.turn_to_heading(90, 1000, 100);
        }

        void no_auton() {
            dt.drive(30, 800, 30, 0);
            dt.brake();
        }
    }

}